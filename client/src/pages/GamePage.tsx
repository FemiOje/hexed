...
export default function GamePage() {
  ...
  // Get token_id from URL (hex string, e.g. "0xfb40...")
  ...
  useEffect(() => {
    if (isDead && !prevIsDeadRef.current && !isMovingRef.current) {
      const reason =
        useGameStore.getState().deathReason || "Slain by another player";
      toast.custom(
        (t) => (
          <div
            style={{
          ...
          >
            ...
            <div style={{ color: "#aaa", fontSize: 12 }}>{reason}</div>
          </div>
        ),
        { duration: 4000 },
      );
  ...
  useEffect(() => {
    ...
    const posChanged =
      prevPos &&
      blockchainPosition &&
      (prevPos.x !== blockchainPosition.x ||
        prevPos.y !== blockchainPosition.y);
      ...
      >
        <div
          style={{
            width: "24px",
            height: "24px",
            border: "2px solid rgba(68, 204, 68, 0.2)",
            borderTop: "2px solid rgba(68, 204, 68, 0.8)",
            borderRadius: "50%",
        ...
        >
          ...
          <div style={{ fontSize: 11, color: "#ffffff", marginBottom: 8 }}>
            {isMoving ? (
              ...
              <>
                <span style={{ color: "#44cc44" }}>Can Move:</span>{" "}
                {canMove ? "Yes" : "Wait..."}
              </>
            )}
          </div>
